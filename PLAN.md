...
-   **用户数据分离 (User Data Separation)**：
    -   将所有可变数据（对话历史、安装的 Skill、配置文件、API Key）存储在标准的用户目录（如 Windows 的 `%APPDATA%\DeepSeekCowork` 或用户指定的 `data` 目录）。
    -   程序本体（exe/zip）可以随意删除、解压覆盖，而不会丢失任何历史记录。
-   **便携模式支持 (Portable Mode)**：
    -   如果在程序同级目录下检测到 `user_data` 文件夹，则优先读取该目录（适合 U 盘携带）。
-   **自动迁移助手 (Migration Assistant)**：
    -   当检测到旧版本数据结构不兼容时，自动运行迁移脚本，确保平滑升级。

### 6.4 极致轻量化 (Software Diet)
随着功能增加，软件体积日益膨胀。V3.1 将致力于打造“小而美”的核心运行时。
-   **按需加载依赖 (Lazy Loading)**：
    -   不再预装所有可能的库（如 `yt-dlp`, `pandas`）。
    -   初始包仅包含核心 Agent 逻辑与基础环境。
    -   当用户需要特定技能时，Agent 自动检测并动态下载/安装所需依赖。
-   **运行时瘦身**：
...
-   **上下文感知 (Context Awareness)**：
    -   能够读取当前浏览器打开的标签页 URL 和标题。
    -   场景：“总结我当前正在看的这篇论文” —— 无需手动复制粘贴链接。
-   **自动化操作 (Browser Automation)**：
    -   通过 Playwright 或浏览器调试协议 (CDP)，控制浏览器执行重复性任务。
    -   场景：“帮我把这 50 个网页都保存成 PDF”、“自动登录并填写今天的日报”。
-   **Cookie/Session 桥接**：
    -   (需用户授权) 安全复用浏览器的登录状态。
...
-   **即时执行模式 (Instant Mode)**：
    -   当前默认行为。适合简单指令（如“列出文件”、“读取代码”）。
    -   特点：低延迟，立即行动。
-   **深度规划模式 (Deep Planning Mode)**：
    -   适合复杂任务（如“重构整个模块”、“从零构建 Web 应用”）。
    -   **Step 1: 提案 (Proposal)**：Agent 分析需求，生成结构化的执行计划（包括步骤、涉及文件、潜在风险）。
    -   **Step 2: 审阅 (Review)**：用户在 UI 上查看计划，可以调整步骤顺序、添加约束或修改目标。
    -   **Step 3: 执行 (Execution)**：获得批准后，Agent 逐步执行计划，并在每个关键节点自动保存检查点 (Checkpoint)，支持随时回滚。
