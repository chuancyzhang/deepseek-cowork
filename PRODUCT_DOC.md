...
6.  **自动化中心**：左侧底部提供独立入口，集中管理已配置任务、执行历史和任务模板。
7.  **会话工具栏**：添加文件、插入智能体、绑定自动化模板、指定能力、开启反问模式都在输入区完成。
8.  **轻量图片理解 / OCR**：图片附件可在支持视觉的模型上直接读取；需要识别截图文字时优先复用模型视觉能力，而不是额外引入本地 OCR 依赖。
9.  **可视化监控**：聊天气泡、工具调用卡片、任务观测详情、子任务状态，以及子 Agent 的结构化过程时间线。
10. **反馈回路入口**：侧边栏提供 `更新长期记忆` 与 `沉淀为 Skill`，由用户决定何时把会话结果转成可复用知识。
...
*   **Core**：
    ...
    *   `sop_manager`：自动化模板、运行态步骤、推进方式解析、确认/重跑/跳过状态机
...
- 会话级自动化模板：每轮只执行当前步骤；模板和步骤都可配置为人工确认或自动推进，系统在应用层逐步调度，而不是把整条流程直接拼成单个执行提示。
- 定时自动化任务：复用同一套模板配置，按计划创建新任务并逐步执行；遇到人工确认步骤时会暂停到等待确认，其余步骤可自动连续推进，并写入执行历史。
