...
13. **启动与运行性能优化**：守护进程连接后台化，点击开始任务、运行代码或继续自动化步骤时不再因为连接重试阻塞 UI。
14. **无黑框子进程体验**：Windows 下常见 Python、Bash、更新 fallback 和系统工具启动统一隐藏控制台窗口，减少 CMD 黑框闪现。
15. **长对话侧边栏快路径**：会话列表和搜索默认只走 SQLite 索引，不再在每次刷新时全量读取旧版 JSON 历史；旧历史可手动迁移。
16. **长回复轻量渲染**：超长助手回复仍完整显示，但会切到轻量纯文本视图，降低打开长会话和滚动聊天区时的 UI 压力。
17. **按需诊断日志**：高频子 Agent/UI 运行诊断默认关闭，需要排查时通过 `COWORK_RUNTIME_DEBUG_LOG=1` 开启。
18. **反馈回路入口**：侧边栏提供 `更新长期记忆` 与 `沉淀为 Skill`，由用户决定何时把会话结果转成可复用知识。
