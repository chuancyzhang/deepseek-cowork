...
20. **长回复轻量渲染**：超长助手回复仍完整显示，但会切到轻量纯文本视图，降低打开长会话和滚动聊天区时的 UI 压力。
21. **按需诊断日志**：高频子 Agent/UI 运行诊断默认关闭，需要排查时通过 `COWORK_RUNTIME_DEBUG_LOG=1` 开启。
22. **反馈回路入口**：侧边栏提供 `更新长期记忆` 与 `沉淀为 Skill`，由用户决定何时把会话结果转成可复用知识。
