...
2.  **技能中心**：双目录技能管理与热加载。
3.  **右侧上下文抽屉**：文件树、SOP、任务观测、子 Agent 监控按需展开，默认收起以保留主对话空间。
4.  **会话工具栏**：添加文件、插入智能体、绑定 SOP、指定能力、开启反问模式都在输入区完成。
...
4.  **结果反馈**
    *   UI 中展示完成状态与详细过程。
    *   右侧上下文抽屉同步展示文件、SOP、任务观测或子 Agent 状态。
...
*   **Core**：
    ...
    *   `sop_manager`：SOP 模板、运行态步骤、确认/重跑/跳过状态机
...
- 会话级 SOP：每轮只执行当前步骤，完成后进入等待确认；用户可在抽屉中确认完成、重跑本步或标记不适用。
...
- 会话层：用户选择的智能体、指定能力和 SOP 会被写入 run context，限制工具可见性并补充当前步骤说明。
