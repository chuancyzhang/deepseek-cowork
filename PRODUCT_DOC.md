...
6.  **自动化中心**：左侧底部提供独立入口，集中管理已配置任务、执行历史和任务模板；定时计划支持 crontab 语法和快捷配置双入口。
7.  **会话工具栏**：添加文件、插入智能体、绑定自动化模板、指定能力、开启反问模式都在输入区完成。
8.  **轻量系统提示条**：聊天流中的系统状态反馈使用紧凑状态条承载，低饱和、自动消失，不打断主阅读路径。
9.  **轻量图片理解 / OCR**：图片附件可在支持视觉的模型上直接读取；需要识别截图文字时优先复用模型视觉能力，而不是额外引入本地 OCR 依赖。
10. **可视化监控**：聊天气泡、工具调用卡片、任务观测详情、子任务状态，以及子 Agent 的结构化过程时间线。
11. **反馈回路入口**：侧边栏提供 `更新长期记忆` 与 `沉淀为 Skill`，由用户决定何时把会话结果转成可复用知识。
