...
13. **Prompt 缓存观测**：任务观测里的系统提示词页区分稳定前缀与每轮 runtime context，展示 prompt / tools / message-prefix 指纹；provider 返回 usage 时同步显示 cached input token 与命中率。
...
## 8. 分层记忆与上下文处理
- 系统层：System Prompt 拆成稳定前缀与每轮 runtime context。稳定策略、工具规范、记忆和思考规范作为会话最前缀保持字节级稳定；工作区、运行模式、日期、运行时路径、指定能力、SOP 步骤和当前工具清单作为请求尾部临时 system message 注入，不写入持久历史，以减少上下文缓存前缀扰动。
