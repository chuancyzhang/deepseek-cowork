...
*   **Workspace Drawer**: A hidden-by-default right context drawer opens from compact icon buttons for files, observability, and sub-agent status. The file page contains both the full workspace tree and a deliverables view that share one preview surface. Once opened, it stays visible while working in the chat and closes only through its close button or `Esc`. The existing visual style is preserved while the narrower project sidebar, 12px main margins, 8px drawer gap, and wider conversation column reduce three-column dead space without overlap. Sub-agent activity lights the panel hint without forcing the drawer open.
