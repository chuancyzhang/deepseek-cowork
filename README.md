...
*   **Multi-format Deliverables**: Existing project file paths in final AI replies are clickable and open directly in the deliverables view inside the right-side file drawer. HTML and Markdown use the embedded web preview, images render inline, PDF uses QtPdf with a built-in text preview when the visual component is unavailable, and DOCX/PPTX/XLSX use built-in structured previews without requiring Microsoft Office. The file page switches between all files and deliverables, while HTML remains the editable source for PPTX, DOCX, and PDF generation in the current conversation.
