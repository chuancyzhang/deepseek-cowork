...
*   **Skill Center import/export/debug** imports custom abilities from a single skill folder, a skill collection folder, or ZIP packages, and exports existing skills individually or as a selected multi-skill collection ZIP. The Skill Center also supports search, enabled-state filters, an icon-free two-column lightweight list, right-side enable switches, user-skill file editing/validation/tool debugging, read-only built-in skills, and MCP tool debugging.
