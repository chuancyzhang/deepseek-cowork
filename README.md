...
*   **Project Sidebar**: The top `新建对话` action always starts a workspace-free chat, while each project row `+` creates a project-bound task. The project `+`, folder, and more-action symbols are drawn by Qt for reliable cross-platform and high-DPI rendering. Production startup applies only the opaque light tooltip surface and native palette, and project rows avoid long hover path tooltips that can render as black blocks on affected Windows hosts. Project headers only expand or collapse their conversations; workspace context follows the visible conversation.
