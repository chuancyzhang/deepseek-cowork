...
- **Natural conversational reasoning**: without in-flight guidance, every reasoning/tool round and stage reply stays inside one assistant-turn container in real execution order, separated only by lightweight dividers. Stage replies remain readable but have no copy, office-draft, or other message actions; only the final answer exposes those actions. In-flight guidance remains a dedicated conversational boundary and starts a new assistant container at its actual send position.
