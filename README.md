...
- **Compact composer tools**: the anchored `+` popover is a single in-window overlay that adds files, prioritizes skills, or captures experience as a Skill. Only one product popover remains active at a time, closed popovers are released, and responsive home-card reflow keeps every card parented inside the main window instead of briefly promoting it to a framed native window.
