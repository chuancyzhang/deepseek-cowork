...
- **Transparent task process**: each run uses one activity timeline for reasoning, tools, response fragments, and in-flight user guidance. Guidance sent during a tool call visibly waits for the current step and changes in place when the next safe boundary applies it. AI text details can fold while tool summaries and user checkpoints remain visible.
