...
*   **Conversation Branching**: Finished user and assistant bubbles expose a lightweight `分支` action that creates a new conversation from that point forward, preserving the workspace and selected skills while recording the parent conversation/message in session metadata.
