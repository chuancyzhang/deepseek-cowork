...
*   **Workspace Drawer**: A hidden-by-default right context drawer opens from compact icon buttons for files, deliverables, observability, and sub-agent status. Once opened, it stays visible while working in the chat and closes only through its close button or `Esc`. The existing visual style is preserved while the narrower project sidebar, 12px main margins, 8px drawer gap, and wider conversation column reduce three-column dead space without overlap. Sub-agent activity lights the panel hint without forcing the drawer open.
*   **Multi-format Deliverables**: Existing project file paths in final AI replies are clickable and open directly in the right-side deliverables drawer. HTML and Markdown use the embedded web preview, PDF uses QtPdf with a built-in text preview when the visual component is unavailable, and DOCX/PPTX/XLSX use built-in structured previews without requiring Microsoft Office. The drawer switches between list and focused-document layouts, while HTML remains the editable source for PPTX, DOCX, and PDF generation in the current conversation.
