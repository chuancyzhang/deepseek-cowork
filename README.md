...
- **Quiet history restore**: deliverables found while reopening stored conversations remain available in the file drawer without being announced as newly generated results.
...
- **Settings center**: manage appearance, models, agents, personality and memory, workspace defaults, archived projects and conversations, MCP servers, enterprise messaging, and runtime components through consistent list/detail pages. Component status uses the last successful snapshot and is probed only after an explicit user action; opening or saving Settings never triggers a scan.
- **AI-configurable workspace theme packages**: Cowork keeps its built-in Linear light theme as a code-only, read-only baseline. Each user theme is one validated `.cowork-theme` package that can carry static raster assets and declaratively style both sidebars, the app header, chat canvas and message templates, composer, and new-chat home. A theme may change the native window caption through `brand.title`, while Windows retains ownership of the title-bar appearance and controls. Left/center/right ownership, actions, accessibility text, and protected in-app controls remain code-defined. Legacy v1 JSON themes still load and migrate only on an explicit save. Preview revisions are isolated and approval-bound.
