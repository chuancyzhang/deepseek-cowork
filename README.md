...
- Optionally enable the default-off `visualize` plugin to embed offline, self-contained interactive charts, explorers, simulations, and UI previews in replies. When disabled, it registers no generation tools and only verified historical artifacts can load lazily in read-only mode.
...
- **Natural conversational reasoning**: each run follows the visible order of user request → deep reasoning with its tools → optional in-flight guidance → a new reasoning segment → final answer. Every reasoning segment folds together with its tool summaries, guidance remains a dedicated conversational row at its actual send position, and any partial answer already shown is preserved as a stage reply.
