...
- **Compact composer tools**: the anchored `+` popover adds files, prioritizes skills, or captures experience as a Skill. Capability search covers display names, internal names, descriptions, and linked tools; the model control always preserves provider, model, and reasoning effort.
...
- **Linear project sidebar**: projects preview the five most recent conversations and retain explicit expand/collapse rows. Projects and conversations support inline rename from menus, context menus, or `F2`, and manual conversation titles persist across later saves.
