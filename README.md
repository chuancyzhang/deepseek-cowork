...
*   **HTML Creation Flow**: The deliverables drawer treats HTML as the editable source rather than another file row. Selecting HTML opens a lightweight live preview that suppresses continuous animation work, preserves vertical and horizontal wheel/trackpad scrolling (including nested scroll areas), reuses unchanged renders, and scans workspace artifacts in the background. Users can force-refresh it, enlarge the preview, drag the drawer width or preview split, and return after switching conversations without losing the selected source. PPTX, DOCX, and PDF actions create a new normal conversation in the same project with the current HTML attached.
