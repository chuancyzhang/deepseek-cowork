...
*   **Project Sidebar**: The top `新建对话` action always starts a workspace-free chat, while each project row `+` creates a project-bound task. Project names shrink and elide before a reserved action area, keeping the Qt-drawn `+` and more buttons visible across narrow sidebars, system fonts, and high-DPI displays. The six project and conversation icon actions use an in-window light function hint instead of Windows native tooltip windows that can render as black boxes. Project headers only expand or collapse their conversations; workspace context follows the visible conversation.
*   **Composer Project Selector**: The selector below the composer is the single project-connection entry point. It can search joined projects, add a project, or choose no project. Only an empty, idle conversation may change its project; conversations with messages retain their existing workspace boundary.
*   **Workspace Drawer**: A hidden-by-default right context drawer opens from compact icon buttons for files, observability, and sub-agent status. The file page contains both the full workspace tree and a deliverables view that share one preview surface. Once opened, it stays visible while working in the chat and closes only through its close button or `Esc`. The existing visual style is preserved while the narrower project sidebar, 12px main margins, and 8px drawer gap keep the conversation column centered at a comfortable reading width when the drawer is closed, then dynamically reserve space when the drawer opens. Sub-agent activity lights the panel hint without forcing the drawer open.
*   **Multi-format Deliverables**: Existing project file paths in AI replies are clickable and also appear as compact file cards beneath the message. After the user opens the deliverables view, newly emitted paths in the same workflow automatically switch the preview to the latest file; changes to the same path still request an explicit refresh. HTML and Markdown use the embedded web preview, images render inline, PDF uses QtPdf with a built-in text preview when the visual component is unavailable, and DOCX/PPTX/XLSX use built-in structured previews without requiring Microsoft Office.
...
*   **Staged Background Startup**: A lightweight loading window appears first after launch, then the main window is built and shown before default workspace hydration, sidebar history rendering, tray setup, daemon prewarm, daemon monitoring, and automation scheduling continue in short background phases to keep launch interaction smooth even on slower machines.
...
*   **Long Conversation Fast Path**: Opening or searching the sidebar stays on the SQLite history index instead of scanning every legacy `chat_history_*.json` file on each refresh.
...
Packaged builds retain the complete sandbox Python base environment. Document, data-analysis, finance, browser-automation, and web-research dependencies can be installed on demand from Settings → Components & Dependencies, using PyPI, Tsinghua, Aliyun, or a custom HTTPS index. Users can queue several component operations; the app processes them one at a time in the background and restores progress and logs when Settings is reopened. Python toolkits are installed into a staging directory and their declared imports are verified before the new version is atomically enabled, so an incomplete dependency tree is never reported as installed. Node.js can use the official source or npmmirror. `PySide6-Addons` and QtWebEngine resources remain bundled for HTML preview.
...
### 2. Select Project
Use the top **新建对话** action for a workspace-free chat. For local file work, use a project row `+`, or select a joined project below the composer while the conversation is still empty. A project remains the workspace boundary for file access.
...
- The context drawer no longer listens for outside clicks; the file page, observability, and sub-agent pages stay open until the user clicks close or presses `Esc`.
