...
*   **Project Sidebar**: The top `新建对话` action always starts a workspace-free chat, while each project row `+` creates a project-bound task. The plus icon is drawn by Qt for reliable cross-platform and high-DPI rendering. Production startup applies only the opaque light tooltip surface and native palette, avoiding black tooltip windows on affected Windows hosts without changing the established desktop appearance. Project headers only expand or collapse their conversations; workspace context follows the visible conversation.
...
*   **Multi-format Deliverables**: Existing project file paths in final AI replies are clickable and open directly in the right-side deliverables drawer. HTML and Markdown use the embedded web preview, PDF uses QtPdf, and DOC/DOCX, PPT/PPTX, and XLS/XLSX use the locally installed Microsoft Office applications to create cached read-only PDF previews. The drawer switches between list and focused-document layouts, while HTML remains the editable source for PPTX, DOCX, and PDF generation in the current conversation.
