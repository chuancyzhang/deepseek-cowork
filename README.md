...
- **AI-configurable workspace theme packages**: Cowork keeps its built-in Linear light theme as a code-only, read-only baseline. Each user theme is one validated `.cowork-theme` package that can carry static raster assets and declaratively style the custom title bar, both sidebars, app header, chat canvas and message templates, composer, and new-chat home. Left/center/right ownership, actions, accessibility text, and protected controls remain code-defined. Legacy v1 JSON themes still load and migrate only on an explicit save. Preview revisions are isolated and approval-bound.
