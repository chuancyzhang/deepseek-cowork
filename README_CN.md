...
- 直接对话默认使用 exe 运行目录下的独立对话工作目录；项目对话把文件操作限制在指定项目工作区内。
...
- **对话 + 项目模型**：直接对话会自动获得 `conversation_workspaces/<session_id>/` 工作目录，也可把空白对话显式绑定到项目工作区；打开、添加、拖入或浏览项目只更新文件视图，不会把已有对话静默迁入该项目。新提交的对话会立即出现在左侧栏，后台保存队列完成后再与 SQLite 历史校准。
- **对话级模型选择**：主输入区模型选择按当前对话保存，表示下一轮要使用的模型；同一对话运行完成后可切换模型继续，运行中的一轮会使用提交瞬间的模型快照，不受后续切换影响。
...
- **Linear 项目侧栏**：项目展开默认显示最近 5 个对话，并保留“展开显示 / 收起全部”；项目、项目内对话和独立对话都可通过更多菜单、右键或 `F2` 就地重命名，手动标题不会再被首条消息自动覆盖。
...
- **轻量启动**：主界面先完成可交互显示，Skill 索引、MCP tool 探测、交付物 WebEngine 预览和历史侧栏更多记录都延后到后台或首次使用时加载。
...
- **能力中心**：在主内容区搜索、筛选、启用、配置、调试和校验能力，无需重启。
...
8. 当某段流程有复用价值时，在 **设置 → 个性与记忆** 维护长期信息，或从会话工具菜单、AI 回复末尾选择 **沉淀为 Skill**；两步向导会选择保存方式和会话片段，再生成可审阅草稿。
