...
- **对话 + 项目模型**：直接对话会自动获得 `conversation_workspaces/<session_id>/` 工作目录，也可把对话绑定到项目工作区；新提交的对话会立即出现在左侧栏，后台保存队列完成后再与 SQLite 历史校准，项目操作采用更轻量的 Apple 风格图标按钮。
...
1. 打开 **设置**，先配置模型服务。
2. 新建使用自动对话工作目录的直接对话，或把对话连接到某个项目。
