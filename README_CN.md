...
> 交付物支持从 AI 回复中的项目文件路径直接打开，并内嵌预览 HTML、Markdown、图片、PDF 及新版 Office 文件。
...
*   **项目式左侧栏**：左侧栏以本地文件夹作为项目；顶部 `新建对话` 始终创建无项目纯对话，项目行 `+` 创建绑定到该项目的任务。项目名称可压缩省略，右侧固定操作区确保 `+` 与更多按钮在窄侧栏、不同系统字体和高 DPI 下仍然可见；六个项目与对话图标按钮使用窗口内浅色功能提示，不依赖可能显示黑框的 Windows 原生提示窗口。点击项目标题只负责展开/收起会话，当前项目始终跟随当前可见对话。
*   **输入栏项目选择器**：输入栏下方是唯一的项目连接入口，可搜索已加入项目、添加新项目或选择“不使用项目”。只有尚无消息且未运行任务的空对话可以切换项目；已有内容的对话保持原项目边界。
*   **右侧上下文抽屉**：文件、任务观测、子 Agent 状态通过紧凑图标按钮按需展开；文件页内置“全部文件 / 交付物”分段，工作区文件树与交付物列表共用同一套预览区。打开后不会因点击聊天区自动收起，只通过右上角关闭按钮或 `Esc` 主动关闭。界面保持原有视觉风格，项目栏收窄至 232px、主区水平边距为 12px、抽屉间距为 8px；抽屉关闭时中间聊天列保持居中的舒适阅读宽度并允许右侧留白，抽屉打开时继续动态避让且保留窄窗口防重叠保护；子 Agent 活动只点亮面板提示，不强制打开抽屉。
*   **多格式交付物预览**：AI 回复中当前项目内真实存在的文件路径可直接点击，回复末尾还会生成轻量文件卡片，并在右侧文件页的交付物视图中打开。用户打开交付物视图后，同一工作流中新输出的文件路径会自动切换到最新交付物；同一路径内容变化仍明确提示用户手动刷新。HTML 与 Markdown 使用内嵌网页预览，PNG/JPG/JPEG/GIF/WEBP/BMP 图片直接内嵌显示，PDF 优先使用 QtPdf，组件不可用时显示内置文本预览；DOCX/PPTX/XLSX 使用应用内结构化预览，不要求用户安装 Microsoft Office。
...
*   **会话级控制**：输入区可将文件作为“用户添加的文件”附件 chip 添加，插入智能体、指定本会话可用能力或开启反问模式；输入框会随内容自动增高并在窄宽度下重新换行，已选能力 chip 可一键移除。
*   **消息原地编辑**：已完成的用户消息可直接切换为带“取消 / 发送”的气泡内编辑器；发送后在当前会话截断后续历史并重新生成，编辑首条消息时欢迎开屏会随新消息收起，删除则只移除当前用户消息、保留后续内容。
*   **对话置顶与归档**：项目内和无项目对话行在悬停时提供置顶、归档按钮；置顶对话优先排序且不会改写最近活动时间，归档后立即从侧边栏隐藏。侧边栏图标的功能名称由窗口内浅色提示气泡显示。
*   **长提问气泡完整展示**：用户气泡会完整展示提问内容，中文长句、长英文标识符、文件名和带连字符的文本都会按宽度自动换行，不再被蓝色气泡裁掉或隐藏。
*   **系统状态提示条**：普通系统提示以主窗口顶部悬浮条显示，使用低饱和底色、自动换行、克制的淡入滑动和自动消失，不再挤压聊天消息；需要人工确认的自动化步骤仍在聊天流里显示确认、重跑和跳过操作条。
...
*   **启动与运行更顺滑**：点击 exe 后会先显示轻量加载窗，再进入主窗口，并分阶段后台补默认工作区、侧边栏历史、托盘、守护进程预热、守护进程监控和自动化调度；点击开始任务或运行代码时不再等待主线程内的多次连接重试。
...
*   **长对话快速路径**：侧边栏只走 SQLite 历史索引，不再每次刷新都全量扫描旧版 `chat_history_*.json`。
...
*   **模型与服务**：按 provider 或用途拆分多个模型服务，每个服务可独立维护访问密钥、服务地址和模型列表，并可使用尚未保存的表单内容测试当前模型。OpenAI 兼容模型可显式配置自身支持的推理强度档位。
...
### 2. 选择项目
顶部 **新建对话** 可随时开始无项目纯对话。需要处理本地内容时，可使用项目行 `+` 创建项目对话，或在空对话的输入栏下方选择已加入项目；Agent 只能在当前对话绑定的项目范围内读写文件。

### 3. 开始自动化
对话栏的紧凑模型菜单使用“渠道 / 模型”标明来源；配置了推理强度能力的模型会在同一菜单显示允许档位，并按模型记住上次选择，没有该能力的模型不会出现推理选项。
示例：
...
*   **任务模板与会话执行**：SOP 模板仍由系统自动生成 ID；模板和步骤都可配置人工确认或自动推进，步骤执行器支持 Agent、上传 Python 文件和 Bash 命令；自动化打开或恢复关联会话后，等待确认的步骤直接在聊天流里操作。
...
### 7. 应用更新
打开设置时会在后台轻量检查 GitHub Releases；发现新版本后，“更新”入口和更新页顶部会显示提醒，但不会自动下载。用户点击 **立即更新** 后，打包版下载并校验完整 Release ZIP，解压后在本机用 SHA-256 比较当前安装目录与新版本，只备份、写入或删除实际发生变化的程序文件，未变化文件不会重复复制，`user_data` 始终保留。独立更新器支持前台或后台安装、失败差异回滚和自动重启。源码运行模式只检查版本并提供 Releases 页面入口。
...
*   **`core/updater.py`**：GitHub Releases 检查、完整包校验、本地文件差异清单和 Windows 差异更新器。
*   **`skills/`**：核心内置技能。
...
- Anthropic 单轮返回多个工具调用时，会把全部工具结果合并到紧随 `tool_use` 的一条用户消息中，满足 `tool_result` 的协议顺序要求。
- 子 Agent 生命周期将结果处理与 Qt 线程清理拆开，旧 worker 完全退出后才会续跑排队输入。
- 子 Agent 监控事件先收集到会话队列，用户打开抽屉后再以轻量摘要行渲染，避免流式过程中频繁构造控件。
- 右侧上下文抽屉不再监听外部点击；文件页、任务观测和子 Agent 页面会持续保持展开，直到用户点击关闭按钮或按下 `Esc`。
