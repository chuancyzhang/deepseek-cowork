...
*   **功能中心导入/导出与调试**：可从单个 Skill 目录、Skill 集合目录或 ZIP 导入自定义能力，也可将现有 Skill 单独导出，或多选后统一导出为可迁移集合 ZIP；能力中心支持搜索、启用状态筛选、无图标双列轻量列表、右侧开关管理，以及自定义 Skill 的文件编辑/验证/Tool 调试。内置 Skill 只读，MCP 能力可直接测试连接并调用 tools。
