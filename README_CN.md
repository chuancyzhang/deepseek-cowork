...
*   **右侧上下文抽屉**：文件、交付物、任务观测、子 Agent 状态通过紧凑图标按钮按需展开；打开后不会因点击聊天区自动收起，只通过右上角关闭按钮或 `Esc` 主动关闭。界面保持原有视觉风格，项目栏收窄至 232px、主区水平边距为 12px、抽屉间距为 8px，中间聊天列进一步利用可用空间且保留窄窗口防重叠保护；子 Agent 活动只点亮面板提示，不强制打开抽屉。
*   **多格式交付物预览**：AI 最终回复中当前项目内真实存在的文件路径可直接点击，并在右侧交付物抽屉中打开。HTML 与 Markdown 使用内嵌网页预览，PDF 优先使用 QtPdf，组件不可用时显示内置文本预览；DOCX/PPTX/XLSX 使用应用内结构化预览，不要求用户安装 Microsoft Office。抽屉可在列表与专注预览间切换。HTML 仍可作为创作源，在当前对话中继续生成 PPTX、DOCX 或 PDF。
