...
*   **任务模板**：继续使用原来的 SOP 模板配置逻辑，但不再暴露手工 ID，系统会自动生成；模板和步骤都可配置人工确认或自动推进。
...
- 支持暂停/恢复/停止，安全管控长任务。
- 会话自动化模板会把执行约束在当前步骤内；模板和步骤都可以配置为人工确认或自动推进，系统每次只派发当前步骤而不是整段 SOP prompt。
- 定时自动化任务会复用同一套模板配置，按步骤创建独立执行轮次；遇到人工确认步骤时会暂停到“等待确认”，其余步骤可自动连续推进，并把每次运行写入本地执行历史。
