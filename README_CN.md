...
- 在 AI 回复末尾点击 **生成办公稿**，把当前回复转成可预览、可迭代的自由、PPT、设计稿或 DOCX 型办公交付物；生成过程默认折叠为任务卡。
- 从 HTML 生成 PPTX 时可选择 PPT 模板，继承主题、母版、字号、色彩、版式节奏以及顶部/底部图片。

## 产品界面

- **对话 + 项目模型**：可直接开始纯对话，也可把对话绑定到项目工作区。
- **生成办公稿**：在 AI 回复末尾选择自由、PPT、设计稿或 DOCX，把内容沉淀为可预览工作稿，生成轮次默认收起为任务卡。
- **右侧上下文抽屉**：按需查看文件、交付物、任务观测和子 Agent 状态。
- **Token 用量提示**：会话顶部显示本对话累计 token 与缓存输入用量，便于随时感知上下文消耗。
