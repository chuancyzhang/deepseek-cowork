...
*   **项目式左侧栏**：左侧栏以本地文件夹作为项目；顶部 `新建对话` 始终创建无项目纯对话，项目行 `+` 创建绑定到该项目的任务。项目区的 `+`、文件夹和更多操作符号使用 Qt 确定性绘制，在不同系统和 DPI 下保持可见；正式启动只应用独立的浅色 Tooltip 表面与原生 Palette，并避免项目行悬停弹出长路径黑框。点击项目标题只负责展开/收起会话，当前项目始终跟随当前可见对话。
