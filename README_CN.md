...
*   **项目式左侧栏**：左侧栏以本地文件夹作为项目，顶部固定 `新建对话` 与搜索；点击项目标题只负责展开/收起该项目下的会话，`+` 按钮才会在该项目中新建对话；右上角 `当前项目` 会始终跟随当前可见对话，而不是跟随刚点开的项目标题，避免运行中的会话和顶部项目标签错位；项目默认折叠并只预览少量会话，点 `展开显示` 后再展开完整列表，正在运行的会话会在对应行显示轻量旋转环，多个后台任务可同时反馈状态；右上角不再提供独立工作区切换入口，整体视觉采用更柔和的 Apple 风格面板。
...
*   **交付物预览**：右侧交付物页会发现工作区里的 HTML、图片、PDF、DOCX、PPTX 和 XLSX 输出；HTML 可在应用内渲染、文件更新后刷新查看，也可基于当前 HTML 新建普通对话，让 AI 调用现有工具生成 PPTX、DOCX 或 PDF。
...
### 2. 选择项目
在左侧栏的 **项目** 区添加本地文件夹。项目就是工作区，Agent 只能在当前对话绑定的项目范围内读写文件；点击项目标题只展开该项目下的会话，切换项目请直接切到对应会话，或使用项目行上的 `+` 新建绑定到该项目的新对话。
...
*   **功能中心导入/导出与调试**：可从单个 Skill 目录、Skill 集合目录或 ZIP 导入自定义能力，也可将现有 Skill 单独导出，或多选后统一导出为可迁移集合 ZIP；能力中心将核心内置能力、默认关闭的随包可选插件、MCP 能力和自定义能力分开管理。内置与随包插件只读不可删除，自定义 Skill 可编辑/验证/调试。自定义能力和随包插件切换后会立即重载运行时能力；MCP 开关会与设置页对应 server 的启用状态同步。开关整条轨道都可点击，且不会误打开能力工作台。
