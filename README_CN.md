...
- **透明任务过程**：Thinking 默认折叠为紧凑过程行，展开后按真实顺序显示原始 reasoning 与工具调用；工具参数、代码、stdout、stderr 和 Traceback 继续在任务观测抽屉查看。历史会话恢复完整顺序但不重播动画。
- **紧凑输入工具栏**：`+` 以锚定 Popover 添加文件、指定能力或沉淀 Skill；指定能力支持名称、内部名、描述与关联工具搜索，多选后以单一能力芯片回显。模型按钮始终显示“渠道 / Model · 思考强度”。
...
- **必要时澄清**：AI 默认直接执行；只有关键意图不清会导致执行错误或明显风险时，才在所属对话中显示内联单选、多选、文本或确认卡。后台请求只更新状态与 Toast，不抢占当前会话焦点。
