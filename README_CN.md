...
### 7. 应用更新
打开 **⚙️ 设置 → 更新** 检查 GitHub Releases。打包版仍下载并校验完整 Release ZIP，解压后会在本机用 SHA-256 比较当前安装目录与新版本，只备份、写入或删除实际发生变化的程序文件，未变化文件不会重复复制，`user_data` 始终保留。独立更新器支持前台或后台安装、失败差异回滚和自动重启。源码运行模式只检查版本并提供 Releases 页面入口。
