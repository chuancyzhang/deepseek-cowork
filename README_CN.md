...
### � 统一消息网关
*   **飞书长连接**: 通过长连接接收事件，无需 Webhook 服务。
*   **飞书命令**: 直接在飞书中下发指令，触发桌面端执行。
*   **按日会话**: IM 对话按日期拆分，便于历史追踪。
*   **工作区约束**: 未开启上帝模式时，IM 指令同样受工作区限制。
...
### 4. 企业消息 (飞书)
打开 **⚙️ 设置 → 企业消息**，填写 **飞书 App ID / App Secret** 并启动网关。IM 对话将遵循工作区限制并按天滚动。
