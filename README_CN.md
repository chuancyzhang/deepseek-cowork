...
- 通过输入栏的 **办公模式** 生成可预览、可迭代的办公交付物，支持自由、PPT、设计稿和 DOCX 类型。

## 产品界面

- **对话 + 项目模型**：可直接开始纯对话，也可把对话绑定到项目工作区。
- **办公模式**：在输入栏常驻开启，围绕可预览工作稿持续协作，并按自由、PPT、设计稿、DOCX 调整输出结构。
...
1. 打开 **设置**，先配置模型服务。
2. 新建纯对话，或把对话连接到某个项目。
3. 让 Agent 读取文件、修改代码、生成报告或执行自动化；需要办公交付物时，可在输入栏开启 **办公模式** 并选择类型。
