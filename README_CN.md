...
*   **项目式左侧栏**：左侧栏以本地文件夹作为项目；顶部 `新建对话` 始终创建无项目纯对话，项目行 `+` 创建绑定到该项目的任务。项目名称可压缩省略，右侧固定操作区确保 `+` 与更多按钮在窄侧栏、不同系统字体和高 DPI 下仍然可见；图标使用 Qt 确定性绘制，并避免项目图标按钮触发部分 Windows 主机上的黑色原生提示框。点击项目标题只负责展开/收起会话，当前项目始终跟随当前可见对话。
