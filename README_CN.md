...
*   **右侧上下文抽屉**：文件、任务观测、子 Agent 状态通过紧凑图标按钮按需展开；文件页内置“全部文件 / 交付物”分段，工作区文件树与交付物列表共用同一套预览区。打开后不会因点击聊天区自动收起，只通过右上角关闭按钮或 `Esc` 主动关闭。界面保持原有视觉风格，项目栏收窄至 232px、主区水平边距为 12px、抽屉间距为 8px，中间聊天列进一步利用可用空间且保留窄窗口防重叠保护；子 Agent 活动只点亮面板提示，不强制打开抽屉。
