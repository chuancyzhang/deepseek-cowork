...
*   **手动反馈入口**：侧边栏提供 `记忆` 与 `沉淀为 Skill`；生成内容在人确认后保存，记忆模块之后仍可随时编辑。
...
*   **`记忆`**：打开分层记忆中心，随时编辑全局/工作区长期摘要、可检索记忆模块和全局灵魂提示词。历史会话生成结果先预览，确认后才保存，并拆成可继续编辑、启停、归档和恢复上一版的模块。
...
- 记忆层：灵魂提示词与全局/当前工作区摘要自动注入；详细记忆模块按需检索，并可在记忆中心随时编辑、启停、归档或恢复。
