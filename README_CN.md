...
技能既可以只提供经验，也可以同时携带工具实现。内置技能位于 `skills/`，随包插件和用户技能位于 `ai_skills/`。技能可以在 `skill.json` 声明 `config_fields`，工作台会自动生成“配置”页，并把保存的值按字段声明注入到脚本或工具运行环境。
...
4. 在 **技能中心** 启用需要的可选能力；腾讯文档、飞书文档、钉钉文档已作为三套独立文档 skill 内置，工作台可查看凭据配置、只读文件、Tool 调试和脚本入口。
