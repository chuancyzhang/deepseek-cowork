...
- **PPT Mode / PPT Agent**：首页卡片和侧栏智能体模块提供内置 PPT Agent 入口，支持自动选择、网页演示、技术分享、高审美商业汇报和模板化办公 PPT 等偏好；内置 Guizang PPT Skill、Frontend Slides、Huashu Design 三类 html-ppt 策略，输出统一注册为 HTML 交付物，再复用现有 PPTX/DOCX/PDF 转换链路。
...
- **设置中心**：通过一致的列表—详情布局管理模型、智能体、默认工作区、归档项目与对话、MCP、企业消息和组件依赖；只有可保存配置值真实变化时才能保存，后台更新日志和连接测试不会误报未保存。
- **首页工具包提示**：新会话首页会提示可在设置中安装文档工具包和数据分析工具包，便于处理 Office/PDF、表格、数据分析和可视化任务。
- **自动化中心**：用紧凑空态、可搜索能力选择和固定操作栏管理提示词任务、定时计划、执行历史、引用能力和可选 Agent 绑定。
- **技能中心**：使用主从布局搜索、筛选、启用、配置、调试和校验技能，无需重启。
