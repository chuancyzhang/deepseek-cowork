...
*   **项目式左侧栏**：左侧栏以本地文件夹作为项目，顶部固定 `新建对话` 与搜索；点击项目即切换工作区，项目默认折叠并只预览少量会话，点 `展开显示` 后再展开完整列表；会话统一从左侧栏切换，不再重复显示顶部标签，正在运行的会话会在对应行显示轻量旋转环，多个后台任务可同时反馈状态；右上角不再提供独立工作区切换按钮，整体视觉采用更柔和的 Apple 风格面板。
