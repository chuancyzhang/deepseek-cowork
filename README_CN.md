...
*   **项目式左侧栏**：左侧栏以本地文件夹作为项目；顶部 `新建对话` 始终创建无项目纯对话，项目行 `+` 创建绑定到该项目的任务。`+` 使用 Qt 确定性绘制，在不同系统和 DPI 下保持可见；正式启动只应用独立的浅色 Tooltip 表面与原生 Palette，避免部分 Windows 主机出现黑框且不改变既有桌面外观。点击项目标题只负责展开/收起会话，当前项目始终跟随当前可见对话。
...
*   **多格式交付物预览**：AI 最终回复中当前项目内真实存在的文件路径可直接点击，并在右侧交付物抽屉中打开。HTML 与 Markdown 使用内嵌网页预览，PDF 使用 QtPdf，DOC/DOCX、PPT/PPTX、XLS/XLSX 调用本机 Microsoft Office 生成缓存的只读 PDF 预览；抽屉可在列表与专注预览间切换。HTML 仍可作为创作源，在当前对话中继续生成 PPTX、DOCX 或 PDF。
