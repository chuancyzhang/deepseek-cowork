...
- **提示附件**：小体积文本附件会随本轮请求内联给模型，图片在支持视觉的模型中按图片内容发送；大文件或非文本文件会明确提示路径、大小和需通过工具读取，不再伪装成已读取内容。
...
- **右侧上下文抽屉**：需要时从窗口右侧展开贴边任务面板。浏览态以“工作区 / 交付物”区分目录树和结果列表，支持搜索、类型筛选、排序及位置恢复；详情态只保留一份文件标题，对适用格式提供“预览 / 源码”，HTML 转换收敛为“生成文件…”菜单。聊天文件卡会直达预览；普通工具生成文件只刷新并提示，不抢占当前阅读，办公交付流程才会自动打开最新结果。
...
- **设置中心**：通过一致的列表—详情布局管理模型、智能体、默认工作区、归档项目与对话、MCP、企业消息和组件依赖；只有存在改动时才能保存，带未保存内容退出会统一确认。
