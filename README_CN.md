...
*   **HTML 连续创作流**：右侧交付物页将 HTML 作为可持续迭代的创作源；选中后使用轻量模式自动渲染，抑制持续动画任务以保持滚动流畅，未变化页面直接复用，工作区产物在后台扫描。用户仍可强制刷新、放大预览、拖动抽屉宽度和上下分区，并在切换会话后恢复当前 HTML。生成 PPTX、DOCX 或 PDF 时会在同一项目中新建普通对话并自动附加当前 HTML。
