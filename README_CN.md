...
- **自然对话式推理**：每次运行按“用户提问 → 深度思考（含工具）→ 补充引导 → 新的深度思考 → 最终回答”呈现。每段深度思考会连同其工具摘要一起折叠；补充引导显示在真实发送位置，并在安全节点到达后原位更新状态；已经展示的正文会保留为阶段回复。
...
- **能力中心**：在主内容区搜索、筛选、启用、配置、调试和校验能力，无需重启；启用后会在当前对话显示“可被 AI 发现”，但不会自动加入该会话的“指定能力”。
