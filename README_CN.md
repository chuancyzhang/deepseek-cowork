...
*   **功能中心导入/导出与调试**：可从单个 Skill 目录、Skill 集合目录或 ZIP 导入自定义能力，也可将现有 Skill 单独导出，或多选后统一导出为可迁移集合 ZIP；能力中心将核心内置能力、默认关闭的随包可选插件、MCP 能力和自定义能力分开管理。内置与随包插件只读不可删除，自定义 Skill 可编辑/验证/调试。自定义能力和随包插件切换后会立即重载运行时能力；MCP 开关会与设置页对应 server 的启用状态同步。
...
- 对 imported / agent skill，系统默认采用持久化延迟全文暴露：先注入 brief，命中 query 或被 `tool_search` 命中后，将完整 skill 内容记录为隐藏会话上下文，后续轮次复用稳定历史前缀，不再重复展开同一内容。
