...
> 交付物支持从 AI 回复中的项目文件路径直接打开，并内嵌预览 HTML、Markdown、PDF 及新旧 Office 文件。
...
*   **多格式交付物预览**：AI 最终回复中当前项目内真实存在的文件路径可直接点击，并在右侧文件页的交付物视图中打开。HTML 与 Markdown 使用内嵌网页预览，PDF 优先使用 QtPdf，组件不可用时显示内置文本预览；DOCX/PPTX/XLSX 使用应用内结构化预览，不要求用户安装 Microsoft Office。文件页可在全部文件与交付物间切换，HTML 仍可作为创作源，在当前对话中继续生成 PPTX、DOCX 或 PDF。
