...
- **子 Agent 观测边界**：子 Agent 事件继续保存到会话状态、右侧观测页和诊断日志；普通对话气泡不创建 Agent 短 ID、状态 PiP 或原始日志容器，只接收需要回传的最终结果。观测页重建事件卡片时，待删除卡片必须保持为内容容器的子控件直到 Qt 延迟销毁完成，禁止通过解除父级把卡片短暂提升为系统顶层窗口。
