...
- 会话级 `ui_timeline_v1` 事件账本继续按原顺序保存 Thinking、工具、正文片段和运行中引导，但 UI 不再把账本等同于一个统一容器。投影层把连续 reasoning 与其触发的工具组成独立“深度思考 · 用时”片段，片段折叠时正文和工具摘要一起隐藏；引导结束当前片段并作为独立轻量行插入，后续 reasoning 创建新片段；已输出正文固定为阶段回复，最终正文始终作为普通 AI 回复。工具开始与结果仍通过同一 `tool_call_id` 原位更新，原始模型消息结构保持不变，历史恢复不重放流式动画。
...
- 运行中引导使用用户消息 `meta.same_turn_guidance` 标记，通过 steer 注入当前 in-flight turn；UI 立即结束当前深度思考片段，并在真实发送位置创建独立 guidance 行，普通状态为“等待下一安全节点”，存在未完成工具时为“完成当前步骤后应用”。运行时 `guidance` observability 事件按消息 ID 将原行更新为“已应用”，后续 reasoning 投影为新的深度思考片段。`ui_timeline_v1` 只保存在会话展示元数据中，并由 `_messages_for_worker` 排除，不改变模型协议；新格式损坏时必须显示恢复警告并记录诊断日志。
...
- 内联可视化由默认关闭的 `ai_skills/visualize` 提供。启用时，Python Runner 才向进程注入会话级 `COWORK_VISUALIZATION_DIR`；发布器要求 UTF-8、2 MB 以内、带根 `id`、无完整文档标签且无外部 URL 的 HTML Fragment，按内容哈希发布并登记到 SQLite。消息层只识别独占行 `::cowork-inline-vis{file="..."}`，并仅加载会话中已登记且哈希一致的文件；Fragment 位于禁网、禁导航、仅允许脚本的 sandbox iframe 中，通过 QWebChannel 保存不超过 64 KB 的展示状态。所有直接创建的 Agent 气泡统一通过会话感知工厂注入 workspace、session、storage 和插件状态；合法指令缺少上下文时显示错误并记录诊断事件，不能静默退化为协议文本。插件关闭时不注册工具、不创建目录、不初始化 WebEngine，历史产物按需只读加载。
