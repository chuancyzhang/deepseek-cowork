...
*   **项目式左侧栏**：本地文件夹作为项目，项目列表由用户配置、最近工作区和会话 `meta.workspace_dir` 合并而来；项目默认折叠，仅预览少量会话，选择项目即切换当前工作区，右上角不再提供独立切换入口，视觉上采用更柔和的 Apple 风格浅色面板。无项目归属的 `对话` 分组也采用相同的预览/展开显示/收起全部节奏。会话页继续由内部 `QTabWidget` 管理生命周期，但 TabBar 永久隐藏，侧栏是唯一导航入口；进程内 `live_activity` 驱动对应会话行的旋转环并支持多个并行任务，持久化的旧 `running` 状态不会在重启后误显示为活动任务。
