...
*   **对话置顶与归档**：项目内及无项目对话共用悬停操作区；`meta.pinned` 控制组内优先排序，`meta.archived` 控制侧边栏过滤，元数据局部更新不会改写 `updated_at`。全局 `QToolTip` 样式与 `QPalette` 共用不透明浅色配色，不使用透明度或圆角窗口规则，避免 Windows 原生 tooltip 合成为黑块。
