...
本设计以 5.0.8 发布基线为准；面向用户的范围、兼容性和验收清单见 [RELEASE_NOTES_5.0.8.md](RELEASE_NOTES_5.0.8.md)。
...
- **右侧上下文抽屉**：顶部项目名/“独立聊天”按钮与相邻文件按钮共用一个会话感知入口；入口以 `SessionState.workspace_dir` 为真值，对比窗口目录和 `QFileSystemModel.rootPath()`，仅在不一致时重新挂载并刷新交付物，避免旧的全局项目浏览状态泄漏到当前会话。文件浏览首页由 SQLite 交付物索引驱动：有明确产物时先显示交付物并按需切到工作区，无产物时直接显示目录树；目录不可用时清除旧根并明确提示，不执行隐式回退。浏览态和详情态之间保留选中路径、筛选、目录展开项和滚动位置。
- **子 Agent 观测边界**：子 Agent 事件继续保存到会话状态、右侧观测页和诊断日志；普通对话气泡不创建 Agent 短 ID、状态 PiP 或原始日志容器，只接收需要回传的最终结果。观测页和聊天工具卡中的动态子 Agent 控件必须从构造第一刻绑定稳定父容器；重建事件卡片时，待删除卡片也必须保持为内容容器的子控件直到 Qt 延迟销毁完成，禁止在创建或清理阶段把任何卡片短暂提升为系统顶层窗口。
- **设置中心**：作为主内容区页面，以紧凑分类导航集中配置外观、模型、智能体、个性与记忆、工作区、MCP、企业消息、更新和运行组件；企业消息配置规范化为最多一个启用渠道，飞书通过后台 `register_app` 设备授权流程创建或更新应用，企微通过官方 `aibot.WSClient` 认证、保活、重连和流式回复，SDK 原始帧不进入诊断日志且回复 URL/Code 必须脱敏；渠道局部操作只提交 `im_gateway` 并重启独立网关，不污染其他设置脏状态；daemon 的 Qt 主线程主动观察服务关闭状态并统一停止监听器、端口和进程，确保单例锁随进程释放，避免企业消息连接到僵尸后台服务；外观页只把内置浅色主题作为只读基线，用户主题使用独立 `.cowork-theme` 包并参与统一脏状态、预览和回滚，同时展示资产清单/缩略信息与结构摘要；主题状态签名只在副本上投影当前编辑器值，控件批量加载期间禁止把半加载界面写回主题草稿，确保各主题的资产与工作区场景彼此隔离；主题文件持久化成功后不因当前 Qt 界面刷新失败而撤销，设置页会明确提示重启载入，其他配置或持久化步骤失败时仍统一回滚；用户尚未创建智能体时显示的“新智能体”模板默认停用，需完成配置并主动启用后才进入可用智能体列表；脏状态比较规范化后的可保存快照，记忆与配置失败时共同回滚。
...
- **主题持久化**：代码内的 Linear 默认主题是只读基线，永不序列化。应用数据 `themes/` 下每个用户主题为 `<id>.cowork-theme`，ZIP 内只有 `manifest.json` 和声明过的 `assets/` 静态图片；`_state.json` 只保存活动主题 ID，`_preview.cowork-theme` 保存隔离预览与 `preview_id + revision`。写入使用同目录临时文件、`fsync` 和原子替换。v1 JSON 启动时只读兼容，用户显式保存时才转换
...
- **语义与工作台注册表**：语义令牌继续按 global、left_sidebar、conversation、composer、right_sidebar、management、feedback、controls、preview_shell 分组。主工作台另有稳定 Surface/Component ID；v2 必须声明唯一 `workspace_scene`，最多四层且顺序为底色、单张图片、程序纹理。网格支持统一原点、主次线和固定 attachment；Surface 只接受 transparent/tint/opaque 材质与受控样式，`ComponentSpec` 只含 visible/icon/style/layout，文案只允许 `ContentSpec` 白名单
- **统一场景与事务应用**：`WorkspaceSceneCanvas` 挂在 `main_splitter` 外层宿主上，在窗口级坐标中一次绘制左/中/右工作区；首页是时间线空状态，滚动区、聊天列和路由切换不持有背景。场景位图按 revision、资产哈希、画布尺寸与 DPR 缓存。`ThemeRuntimeManager` 先完整验证 manifest 与资产，再更新令牌和 QSS，由 `WorkspaceThemeController` 原位应用场景、材质、布局、显隐、图标和文案。任何阶段失败都会恢复上一份令牌、场景、样式、布局与组件状态，并记录失败组件和区域耗时；损坏的活动主题显示默认基线和原始错误，但不静默改写活动状态
...
- AI 正文控件关闭自身滚动条并按完整文档高度撑开，不设置 20,000px 截断；富文本与纯文本正文视口将滚轮事件（含高精度像素增量）显式转发给 `ChatScrollArea`，避免隐藏的内部滚动范围抢占输入。聊天区使用单一外层滚动条，将细滑块的视觉宽度与透明命中宽度、邻近抓取容错分离，并保留按压期间暂停自动滚底的状态机。
