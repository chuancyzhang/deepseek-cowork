...
1. **UI 层**
   `main.py` 提供对话界面、项目侧栏、右侧上下文抽屉、设置中心、自动化中心和技能中心；`core/theme.py` 的 `DesignTokens` 是颜色、状态和基础几何的唯一主题来源。
2. **Agent 层**
...
- **右侧上下文抽屉**：文件、交付物、任务观测、子 Agent 状态默认隐藏，按需展开为无阴影贴边面板；文件区域在浏览态和详情态之间切换，聊天交付物入口直接进入详情态。
