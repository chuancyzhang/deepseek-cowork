...
*   **系统提示条**：`add_system_toast(...)` 将普通状态反馈加入主窗口级 FIFO 队列，在主内容顶部单条悬浮展示；提示对齐当前对话列，限制最大宽度、允许换行，并通过克制的淡入滑动与淡出动画自动轮播，不再插入或滚动聊天布局。显式关闭时长保持有效，未指定时普通提示默认 4 秒、错误提示默认 8 秒。会话自动化等待人工确认的交互操作条仍保留在聊天流中，承载确认、重跑和标记不适用。
...
*   **分阶段后台启动**：入口先显示轻量加载窗并把主窗口构造调度回 Qt 事件循环；主窗口显示后再延后补默认工作区、侧边栏历史、托盘、daemon 预热、daemon monitor 和自动化调度，减少点击 exe 后的无反馈等待和首屏阶段的同步负载。
...
*   **更新模式**：源码模式只检查 GitHub Releases；可执行模式下载完整 ZIP、校验并暂存，在本机计算文件差异后通过独立脚本关闭旧进程，只替换发生变化的文件并重启。
