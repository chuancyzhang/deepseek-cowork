...
*   **项目式左侧栏**：本地文件夹作为项目，项目列表由用户配置、最近工作区和会话 `meta.workspace_dir` 合并而来；项目标题点击只负责展开/收起该项目下的会话预览，项目行 `+` 按钮用于在该项目中新建会话。右上角 `当前项目`、文件树和交付物面板始终跟随当前可见会话绑定的 `workspace_dir`，不会被单纯的项目浏览动作切走。无项目归属的 `对话` 分组也采用相同的预览/展开显示/收起全部节奏。会话页继续由内部 `QTabWidget` 管理生命周期，但 TabBar 永久隐藏，侧栏是唯一导航入口；进程内 `live_activity` 驱动对应会话行的旋转环并支持多个并行任务，持久化的旧 `running` 状态不会在重启后误显示为活动任务。
...
- **迁移复用与调试**：功能中心支持 ZIP 导出/导入，并提供搜索、启用状态筛选、无图标双列轻量列表、Apple 风格滑动开关、选择模式批量导出/删除和能力工作台；能力中心拆分为 `内置能力`、`可选插件`、`MCP`、`自定义能力` 四个 tab，自定义 Skill 可编辑/验证/调试/删除，内置 Skill 只读且不可关闭或删除。自定义 Skill 与可选插件开关会立即重载运行时注册表，MCP 开关同步 `mcp_servers[].enabled` 并保留关闭后的列表入口。
...
- **交付物预览与转换**：右侧 `交付物` 抽屉按修改时间列出工作区产物；HTML 支持渲染、刷新、打开、显示位置和复制路径。用户选择 HTML 后点击生成 PPTX/DOCX/PDF 时，UI 会创建一个普通对话并附加源 HTML，由 AI 继续使用现有工具链生成办公文件，而不是在抽屉内写死转换逻辑。
