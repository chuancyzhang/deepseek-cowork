...
- Thinking、工具、正文片段和运行中引导由会话级 UI 事件账本投影为统一任务活动时间线。reasoning/content delta 只更新当前开放事件，工具开始与结果通过同一 `tool_call_id` 原位更新；折叠只隐藏 AI 文本详情，不隐藏工具摘要和用户引导。原始模型消息结构保持不变，历史恢复不重放流式动画。
...
- 运行中引导使用用户消息 `meta.same_turn_guidance` 标记，通过 steer 注入当前 in-flight turn；UI 立即在任务活动时间线创建 guidance 检查点，普通状态为“等待下一安全节点”，存在未完成工具时为“完成当前步骤后应用”。运行时 `guidance` observability 事件按消息 ID 将原检查点更新为“已应用”。`ui_timeline_v1` 只保存在会话展示元数据中，并由 `_messages_for_worker` 排除，不改变模型协议；新格式损坏时必须显示恢复警告并记录诊断日志。
