...
- **办公模式**：输入栏常驻的会话级 workflow 开关，按自由、PPT、设计稿、DOCX 注入办公交付策略。
...
1. 读取当前会话、工作区和运行模式。
2. 按需注入稳定系统策略、记忆摘要、会话 workflow 和最小技能信息。
3. 由模型直接调用工具。
4. 把工具结果回传给模型继续推理。
...
- 办公模式使用 `workflow_mode = office_html_first` 和 `office_output_profile` 注入提示，不新增 `RUN_MODE`，因此不改变工具权限。
