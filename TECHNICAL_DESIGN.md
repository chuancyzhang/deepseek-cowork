...
- **生成办公稿**：AI 回复末尾的消息级动作，按自由、PPT、设计稿、DOCX 注入办公交付策略，并把该生成轮次默认折叠为任务卡。
- **右侧上下文抽屉**：文件、交付物、任务观测、子 Agent 状态默认隐藏，按需展开。
...
- 生成办公稿使用本次请求级 `workflow_mode = office_html_first` 和 `office_output_profile` 注入提示，不新增 `RUN_MODE`，因此不改变工具权限。
- UI 通过用户消息 `meta.workflow_mode = office_html_first` 将该轮用户消息、工具调用和助手结果渲染成可展开的办公稿任务卡；消息本体仍按原结构持久化。
- 从 HTML 生成 PPTX 时可附加 PPTX 模板文件，提示要求以 HTML 为内容源、模板为视觉结构源，并保留模板主题、母版、字号、色彩、版式节奏和顶部/底部图片。
