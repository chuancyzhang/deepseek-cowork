...
- UI 通过用户消息 `meta.workflow_mode` 将该轮用户消息、工具调用和助手结果渲染成可展开的办公任务卡；消息本体仍按原结构持久化，交付物文件卡渲染在折叠过程外。任务卡会合并最终回复、隐藏气泡识别到的文件卡路径和本轮工具变更中的有效工作区文件，避免最终回复未写完整路径时结果区为空。任务刚提交时会写入用户请求、附件、选中 Skill 和启动阶段节点；收到系统提示词、Skill 上下文、thinking、工具调用和工具结果后继续追加到过程区，避免展开过程只显示占位。历史消息按 render span 独立渲染，避免普通消息和办公任务混排时丢失折叠状态。
