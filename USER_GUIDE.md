...
4. 填写模型配置并确认。截图以 GPT‑5.6 Sol 为例：
...
未选择文件夹（项目）时，当前会话为独立聊天。应用默认在应用数据目录下创建 `conversation_workspaces/<session_id>/`：安装版位于 `%APPDATA%\DeepSeekCowork`，便携版位于 exe 旁的 `user_data`。可在 **设置 → 工作区 → 独立聊天目录** 修改后续新会话的根目录；已有会话继续使用原目录，避免文件被静默移动。AI 的文件读取、创建和修改会限制在这个聊天工作目录内，不会直接进入你的项目文件夹。
...
重新打开历史会话时，应用会恢复草稿、滚动位置、右侧抽屉和每段深度思考的展开状态，并把所有既有消息按当前统一轮次规则重新组织。没有补充引导时，同一轮的多段 **深度思考 · 用时**、工具、阶段回复和最终回答收纳在一个 AI 轮次容器中，并按真实顺序排列；每段思考都可独立展开，阶段之间使用紧凑留白和从思考标题起点开始的短细分隔线，不显示阶段编号。首轮内容与后续流式更新会保持顶部对齐并连续向下生长，窗口中的剩余高度留在消息流末尾；工具行的悬停、焦点和选中背景只覆盖该行。没有推理文字或工具的展开区不会绘制空时间线或撑开对话。阶段回复不显示 **复制结果**、**生成办公稿** 等消息动作，只有正文非空的最终回答显示操作栏；任务停止、运行错误或没有最终正文时会显示明确状态，但不会把阶段回复当作最终结果。补充引导仍作为明确边界结束当前容器，并在引导后开启新的 AI 容器。编辑历史用户消息时，消息会原位变成编辑器；选择 **保存并重新生成** 会替换该消息之后的对话内容，但不会删除已经生成的工作区文件。
...
- **Agent、指定能力与 Skill 沉淀**：输入工具栏的 **Agent** 按钮用于把内置或自定义智能体加入当前输入。点击 `+` 会打开唯一的锚定操作面板，可添加文件、搜索并多选当前会话优先能力，或选择 **沉淀为 Skill**；切换到其他面板时原面板会关闭，重复点击不会累积或闪现带标题栏的小窗口。模型入口与 Agent、能力保持单一选择状态。Skill 两步向导会选择保存方式和会话片段，随后在后台生成草稿；完成后点击对话中的 **Skill 草稿待确认** 再查看并保存。
- **能力启停**：在能力中心启用或关闭能力后，后台会立即刷新，但不会在对话中插入提示。启用只表示 AI 可通过发现机制找到它，不会自动改变当前会话的“指定能力”；若后台刷新失败，界面会保留已保存配置并显示原始失败原因。对于 Airflow、Superset 等 Skill 托管的 MCP，保存 Skill 配置会自动生成并启用连接，不需要再进入 MCP 设置页；“测试连接”作为独立诊断动作，成功时显示工具数量，失败时显示认证、网络、TLS 或远端服务状态。Airflow stdio 由 Cowork 按需启动；远程 Superset MCP 未运行时需要管理员在 Superset 环境启动服务。
- **浏览器自动化**：`browser-automation` 默认关闭。启用后若 Tencent BrowserSkill 未就绪，选择“前往设置”会打开 **设置 → 组件与依赖 → 可选浏览器能力**。先点击“安装”下载并校验 `bsk` CLI，再点击“安装浏览器扩展”由用户在 Chrome/Edge 中确认，最后点击“检查连接”。检查过程会短暂打开 Agent Window，并在基础连接检查后执行一次仅限 Agent Window 的标签页读取；只有扩展连接和真实执行通道都正常才显示“已就绪”。AI 随后可读取登录态网页、提取数据、填写表单或代为操作；提交、发送、购买、删除等重要动作仍需用户确认。
...
- **对话内交互可视化**：在能力中心启用默认关闭的 `visualize` 后，可要求 AI 生成交互图表、数据探索器、参数模拟器或 UI 预览。视图默认自包含，也可加载内置受信 CDN 白名单中的静态脚本、样式、字体或图片；不支持任意来源和运行时数据请求。若资源加载失败，卡片会显示被拦截或加载失败的具体原因。筛选和选择状态可随会话恢复；关闭插件后不会再生成新视图，已登记且完整的历史视图仍会以“只读”卡片显示。
...
4. 点击主界面的文件入口后，如果已有生成或标记的结果，会先看到紧凑的 **交付物** 列表，并可通过 **浏览工作区** 进入真实目录树；尚无交付物时会直接显示工作区。工作区文件可从右键菜单标记或取消标记为交付物。选择生成的 `.html` 文件后会下钻到预览与操作态，可在 **预览 / 源码** 间切换。预览区支持滚轮、触控板和可拖动的横纵滚动条。独立聊天目录或项目目录为空时，右侧会隐藏搜索与筛选并显示居中的空状态；点击 **在资源管理器中打开** 可进入当前聊天的专属目录或项目目录。搜索或筛选没有结果时，相关控件会继续保留，并可直接清除筛选。
...
聊天记录右侧使用加宽滑块，可直接按住拖动。超长 AI 回复不会在消息内部形成第二条滚动链，也不会在固定高度被截断，而是完整展开并由整个聊天窗口统一滚动。任务仍在输出时，手动拖动会暂停自动滚底；回到底部后自动跟随恢复。
