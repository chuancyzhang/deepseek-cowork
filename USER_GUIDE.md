...
- **浏览器自动化**：`browser-automation` 默认关闭。启用后若 Tencent BrowserSkill 未就绪，选择“前往设置”会打开 **设置 → 组件与依赖 → 可选浏览器能力**。先点击“安装”下载并校验 `bsk` CLI，再点击“安装浏览器扩展”由用户在 Chrome/Edge 中确认，最后点击“检查连接”。状态变为“已就绪”后，AI 才能在独立 Agent 窗口读取登录态网页、提取数据、填写表单或代为操作；提交、发送、购买、删除等重要动作仍需用户确认。
