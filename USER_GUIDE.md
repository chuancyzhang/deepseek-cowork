...
> 当前界面截图基于 4.9.9 的浅色 Linear 风格工作台。后续版本的按钮位置或文案可能略有调整，请以实际界面为准。
...
重新打开历史会话时，应用会恢复草稿、滚动位置、右侧抽屉和本次运行中主动展开的过程节点。Thinking 默认显示为紧凑的 **深度思考 · 用时** 行；展开后可按真实顺序查看 reasoning 和工具调用，并从工具行进入任务观测详情。编辑历史用户消息时，消息会原位变成编辑器；选择 **保存并重新生成** 会替换该消息之后的对话内容，但不会删除已经生成的工作区文件。

![原位编辑历史用户消息](images/user-guide/36-history-message-edit.png)

![展开 Thinking 与工具过程](images/user-guide/37-thinking-expanded.png)
...
6. 确认工作稿无误后，在底部“继续创作”栏点击 **生成文件…**，再选择 PPTX、DOCX 或 PDF。生成任务会在原位显示目标格式、中文运行状态和取消入口，并阻止重复提交；完成、警告和错误提示会在主窗口右下角堆叠显示，可手动关闭，鼠标悬停时不会消失。选择 PPTX 后可按需选择公司或项目 `.pptx` 模板；AI 会以 HTML 为内容源，以模板为视觉结构源，尽量继承主题、母版、字号、色彩、版式节奏，以及模板顶部和底部的图片元素。
