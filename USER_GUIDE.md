...
本指南介绍 DeepSeek Cowork 的安装、模型配置、基础使用方式，以及如何通过办公模式生成可编辑的 PPTX 文件。

> 当前界面截图基于 4.9.4 版本。后续版本的按钮位置或文案可能略有调整，请以实际界面为准。
...
下面以“开启办公模式，先预览工作稿，再转换为 PPTX”为例，展示一个完整工作流。

1. 选择一个文件夹作为当前项目。

   ![连接用于生成交付物的项目](images/user-guide/14-project-workspace.png)

2. 在输入栏下方开启 **办公模式**，类型选择 **PPT**。也可以点击空对话中的 **办公交付物** 示例卡，再按需要修改提示词。

   ![生成 HTML 交付物示例](images/user-guide/15-generate-html-example.png)

3. 让 AI 在工作区中生成一个或多个可预览工作稿。办公模式开启时，新生成的交付物会自动打开右侧 **文件** 抽屉中的 **交付物** 视图，并进入专注预览。
...
6. 确认工作稿无误后，选中目标文件并点击 **生成 PPTX**。AI 会基于该工作稿生成可编辑的 PowerPoint 文件。
