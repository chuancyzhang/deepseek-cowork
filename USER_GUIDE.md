...
- **Agent、指定能力与 Skill 沉淀**：输入工具栏的 **Agent** 按钮用于把内置或自定义智能体加入当前输入。点击 `+` 会打开唯一的锚定操作面板，可添加文件、搜索并多选当前会话优先能力，或选择 **沉淀为 Skill**；切换到其他面板时原面板会关闭，重复点击不会累积小窗口，首页快捷卡片也不会在面板打开或布局调整时闪现为独立窗口。模型入口与 Agent、能力保持单一选择状态。Skill 两步向导会选择保存方式和会话片段，随后在后台生成草稿；完成后点击对话中的 **Skill 草稿待确认** 再查看并保存。
