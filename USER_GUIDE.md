...
重新打开历史会话时，应用会恢复草稿、滚动位置、右侧抽屉和任务活动时间线。每次运行只有一个 **任务过程 · 用时** 容器，按顺序展示思考、工具、回复片段和补充引导；展开思考可查看 reasoning，工具行可进入任务观测详情。折叠 AI 细节不会隐藏用户补充的引导。编辑历史用户消息时，消息会原位变成编辑器；选择 **保存并重新生成** 会替换该消息之后的对话内容，但不会删除已经生成的工作区文件。

任务运行时仍可在输入区发送补充引导。普通处理中显示 **等待下一安全节点**；如果工具已经启动，则显示 **完成当前步骤后应用**，工具不会被取消或移动。到达安全节点后，同一条引导会原位变为 **已应用**。若 AI 已开始输出正文，引导前的文字会保留为回复片段，后续过程继续排列在引导之后。
...
![任务活动时间线中的补充引导](images/user-guide/38-guidance-timeline.png)
