...
重新打开历史会话时，应用会恢复草稿、滚动位置、右侧抽屉和每段深度思考的展开状态。对话按 **用户提问 → 深度思考 → 补充引导 → 新的深度思考 → 最终回答** 排列；连续 reasoning 和它触发的工具归入同一段 **深度思考 · 用时**，默认折叠，展开后可查看 reasoning，并从工具行进入任务观测详情。折叠该段时，思考正文与工具会一起隐藏，最终回答始终保持可见。编辑历史用户消息时，消息会原位变成编辑器；选择 **保存并重新生成** 会替换该消息之后的对话内容，但不会删除已经生成的工作区文件。
...
- **能力启停即时反馈**：在能力中心启用或关闭能力后，后台确认刷新成功，当前对话会出现一条不进入模型上下文的能力事件。启用只表示 AI 可通过发现机制找到它，不会自动改变当前会话的“指定能力”。若后台刷新失败，界面会保留已保存配置并显示原始失败原因。
