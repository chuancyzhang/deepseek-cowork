...
4. 点击主界面的文件入口会先进入浏览态；“工作区”显示可展开的真实目录树，“交付物”支持搜索、类型筛选和排序。选择生成的 `.html` 文件后会下钻到预览与操作态，可在 **预览 / 源码** 间切换。预览区支持滚轮、触控板和可拖动的横纵滚动条；响应式页面仅在内容确实超出当前宽度时显示横向滚动条。需要在独立浏览器中查看、在资源管理器中定位或复制路径时，使用右上角更多菜单。
...
6. 确认工作稿无误后，在底部“继续创作”栏点击 **生成文件…**，再选择 PPTX、DOCX 或 PDF。生成任务会在原位显示目标格式、运行状态和取消入口，并阻止重复提交；右侧抽屉仍可关闭或切换，完成后会通过 Toast 和交付物入口提示结果。选择 PPTX 后可按需选择公司或项目 `.pptx` 模板；AI 会以 HTML 为内容源，以模板为视觉结构源，尽量继承主题、母版、字号、色彩、版式节奏，以及模板顶部和底部的图片元素。
