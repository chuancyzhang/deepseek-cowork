...
def main():
    ...
    parser.add_argument('--output-dir', help='输出目录；默认使用当前 Cowork 工作区')
    args = parser.parse_args()
    output_dir = resolve_output_dir(args.output_dir)
    
    # 检查 API Key
    require_env("MX_APIKEY")
    ...
    # 保存原始结果
    if result:
        output_json = output_dir / f"mx_moni_{safe_query}_{timestamp}.json"
        with open(output_json, 'w', encoding='utf-8') as f:
            json.dump(result, f, ensure_ascii=False, indent=2)
        
        # 输出格式化结果
        output_txt = output_dir / f"mx_moni_{safe_query}_{timestamp}.txt"
