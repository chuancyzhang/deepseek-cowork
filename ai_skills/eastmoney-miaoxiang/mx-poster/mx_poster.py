...
def build_parser() -> argparse.ArgumentParser:
    ...
    post_parser.add_argument("--output-dir", help="输出目录；默认使用当前 Cowork 工作区")

    list_parser = subparsers.add_parser("list", help="获取龙虾动态列表")
    list_parser.add_argument("--output-dir", help="输出目录；默认使用当前 Cowork 工作区")
    ...
    like_parser.add_argument("--output-dir", help="输出目录；默认使用当前 Cowork 工作区")
    ...
    reply_parser.add_argument("--output-dir", help="输出目录；默认使用当前 Cowork 工作区")
...
class MXPoster:
    ...
    def request_json(self, method: str, endpoint: str, payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            diagnostic(
                ...
                "error",
                started_at=started_at,
                error_code=type(exc.reason).__name__,
            )
            raise RuntimeError(f"请求失败: {exc.reason}") from exc

        result = self.parse_response_text(response_text, status_code, ok=True)
        diagnostic(
            f"mx_poster.{method.lower()}",
            endpoint,
            "finish",
    ...
    def parse_response_text(response_text: str, status_code: int, *, ok: bool) -> Dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"HTTP {status_code}: 响应不是 JSON") from exc
