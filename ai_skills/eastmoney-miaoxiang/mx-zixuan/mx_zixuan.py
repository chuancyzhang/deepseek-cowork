...
支持查询、添加、删除东方财富自选股
Cowork 适配后统一输出到当前会话工作区的 mx_data/output/
"""
...
QUERY_ENDPOINT = "/api/claw/self-select/get"
MANAGE_ENDPOINT = "/api/claw/self-select/manage"
...
def get_apikey() -> str:
    """仅从 Cowork 注入的环境变量获取 API Key。"""
    return require_env("MX_APIKEY")


def request_json(operation: str, endpoint: str, apikey: str, payload: Dict) -> Dict:
...
def query_self_select(apikey: str) -> Dict:
    """查询自选股列表"""
    return request_json("mx_zixuan.query", QUERY_ENDPOINT, apikey, {})

def manage_self_select(apikey: str, query: str) -> Dict:
    """添加或删除自选股"""
    return request_json("mx_zixuan.manage", MANAGE_ENDPOINT, apikey, {"query": query})
