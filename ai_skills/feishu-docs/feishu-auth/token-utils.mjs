...
function getPendingDir(openId) {
  const stateRoot = process.env.FEISHU_AUTH_STATE_DIR
    || (process.env.COWORK_SANDBOX_ROOT ? path.join(process.env.COWORK_SANDBOX_ROOT, 'feishu-auth') : null)
    || path.join(os.homedir(), '.cowork', 'feishu-auth');
  return path.join(stateRoot, '.tokens', openId);
}
