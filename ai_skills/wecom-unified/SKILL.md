...
allowed-tools: wecom_reference_read, wecom_cli
---
...
Cowork 固定提供并校验 `wecom-cli 1.1.0`，凭据由能力商店隔离管理。禁止执行 npm 安装、查找系统 PATH 中的 CLI、切换到全局 CLI，或在对话中运行任何 `auth` 命令。

1. 先调用 `wecom_reference_read` 读取相关业务域入口；入口引用子文档时再按需读取，禁止猜测命令参数。
2. 业务操作只通过 `wecom_cli` 传入参数数组，不经过 shell。
3. 工具返回未授权、需要重新授权或 CLI 不可用时，停止业务操作并引导用户前往「能力商店 → 企业微信办公套件」扫码连接或修复安装。
4. `auth show --status` 只代表本机存在凭据，不代表服务端权限健康；真实连接验证由能力商店统一执行。
