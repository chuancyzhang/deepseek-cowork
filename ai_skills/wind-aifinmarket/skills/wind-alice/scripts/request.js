...
function getApiKey() {
  const apiKey = process.env.WIND_API_KEY?.trim();
  if (apiKey) return apiKey;
  die("KEY_MISSING", "WIND_API_KEY 未通过 Cowork 能力中心配置");
}
...
function usage() {
  return [
    ...
    "  WIND_API_KEY                必需；由 Cowork 能力中心在运行时注入",
    "",
    "Config:",
    "  只使用 Cowork 能力中心配置，不扫描用户目录或 Skill 源码目录。",
  ].join("\n");
}
...
/**
 * Alice 下载目录固定在当前 Cowork 工作区，缺少工作区时直接失败。
 */
export function resolveDownloadDir() {
  const workspace = process.env.COWORK_WORKSPACE_DIR?.trim();
  if (!workspace) {
    throw new Error("COWORK_WORKSPACE_DIR is required for Cowork-managed Alice downloads.");
  }
  return {
    dir: join(resolvePath(workspace), "wind-aifinmarket", "alice-downloads"),
    source: "cowork-workspace",
  };
}
 ...
 *
 * 目标目录由 resolveDownloadDir 决定，目录创建失败时直接返回根因。
 *
 * 重复调用幂等：调用即清空累计列表。
 */
async function downloadCollectedFiles(apiKey) {
  ...
  try {
    ...
    throw new Error(`创建 Cowork Alice 下载目录失败：${targetDir}（${e.message}）`);
  }
...
function dieKeyMissing() {
  die("KEY_MISSING", "WIND_API_KEY 未配置或已失效", {
    extraHint:
      `请在 Cowork 能力中心更新 WIND_API_KEY 后重试。获取入口：${WIND_AIFINMARKET_PORTAL}`,
  });
}
