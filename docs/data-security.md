...
“检查中”“未发现明显风险”“发现风险线索”“检查未完成”表示检查状态，不是安全认证。网络和命令执行本身不代表恶意。能力详情就近展示最近检查状态；“安全记录”用中文展示处理结果和下一步建议；完整范围、文件位置与技术原因可展开“查看详情”。记录按时间从新到旧排列，每页 5 条，最多显示最近 60 条，可筛选“只看需要关注的记录”。阅读和翻页时不自动刷新，点击“刷新记录”获取最新结果。记录在本机按容量自动轮换，不会无限增长。有检查任务时显示“停止检查”；本次应用运行已有检查项目时，可按已保存配置“重新检查 Skill / MCP”。应用启动时不全盘扫描。
