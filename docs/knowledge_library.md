...
- 腾讯文档和乐享使用短时内存缓存：目录及搜索结果 60 秒，正文 120 秒，按来源、账号、企业与授权代次隔离。缓存不写入磁盘，退出应用即释放。点击“刷新”会清除当前来源缓存并重新获取当前目录或搜索结果；重新连接、断开连接、授权/权限失败及上传写入也会清除缓存。连接验证、资料归属核对与上传进度始终查询远端；不会在请求失败时静默显示过期结果。缓存不改变模型系统提示或工具定义。
