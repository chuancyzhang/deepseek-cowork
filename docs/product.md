...
| 能力入口 | 新对话首页、AI 能力商城、Agent、常用 | 需要专业流程或复用工作模式时 |
| 系统管理 | 模型、主题、记忆、企业消息、组件、高级能力管理 | 首次配置、连接或排错时 |

主界面因此围绕四个高频起点组织：PPT、金融分析、数据分析和浏览器
...
| 常用 | 保存提示词、能力组合和执行位置；可选计划在指定时间交给同一 Agent Loop |
| 记忆与经验 | 影响判断和方法，不建立第二套执行协议 |

统一执行面带来五个产品结果：

- **可用**：常规完整启动时，符合当前上下文的核心内置 Tool 首轮直接可见；显式选择的可选能力同步进入本轮，其他已启用扩展再通过 `tool_search` 按需发现。可选极简启动的首请求使用单独的精简上下文。
...
- 两条以上顶层用户提问会在左侧栏旁显示历史提问细轨；标记随滚动更新，悬停显示本轮答复摘要，并可跨未渲染历史分页定位。

### 4.2 文件与交付物

- 文件管理器中的文件可以直接粘贴为附件；剪贴板位图进入会话托管附件。
- “文件与交付物”使用单一工作台：导航未固定时贴住工作台左边缘覆盖内容，点击图钉后放大并固定为左列。
- 同一会话可保留多个运行时文件标签；重复打开只激活已有标签，切换会话后恢复各自的标签集合。
...
- 文件名由顶部标签承载，工具栏只显示一行必要元信息；预览与编辑通过铅笔/眼睛按钮切换，只读格式明确标记且不显示编辑入口。
...
- 常用库保存提示词、能力组合和聊天/项目执行位置，点击后始终新建主智能体任务；定时计划是常用项上的单一可选附加能力。
- AI 可通过 `tool_search` 发现常用库管理 Skill，读取、创建、修改和启动常用项；启动与删除保持确认边界。
...
- 资料库支持 WeKnora、腾讯文档和乐享。登录不等于自动向模型提供资料；引用范围由用户选择，任务保留启动时绑定的账号、来源和范围。
- “账号与连接”集中管理可选认证及能力授权；原配置继续有效，选择统一连接后不会在失败时悄悄换回原凭据或其他账号。认证等待保留消息与成果，由用户认证后继续或取消。
- 数据安全随应用附带且全部默认关闭。能力风险检查、疑似凭证提醒和请求文本令牌化分别启用；不声称覆盖任意第三方外发，也不改变执行权限。
- 极简启动是受支持 DeepSeek 官方模型的新对话选项，先使用精简上下文；显式资料引用、已有会话和专用流程使用完整能力。它不承诺更快或更便宜。
- 上帝模式是后续任务的全局授权默认值，运行中权限冻结；停止向受支持工具传递取消，保留输出，不撤销已经发生的外部操作。
- 用量浮窗用于查看已报告的用量与上下文组成；日志和聊天记录可导出用于排错，导出不等于匿名化。

操作入口见[用户指南](user-guide.md)，边界见[资料库](knowledge_library.md)、[账号与连接](account-connections.md)、[数据安全](data-security.md)和[执行授权](GOD_MODE.md)。
