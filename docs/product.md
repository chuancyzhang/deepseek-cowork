...
1. **可发现**：核心内置 Tool 首轮直接可用；显式选择的可选能力同步直出，其他已启用扩展可通过 `tool_search` 按需出现。
2. **可治理**：运行时能区分只读、写入、危险操作和必须交互的动作。
3. **可观察**：工具参数、状态、结果、错误和能力暴露来源可以投影到同一任务观测界面。
...
经验是本地、显式、可查看、可编辑的运行知识层；这不是模型微调，也不涉及模型权重调整。
...
- **上下文抽屉**：浏览文件、预览或安全编辑交付物，并查看任务观测和子 Agent 状态。
- **AI 能力商城**：通过任务场景和典型任务发现能力，以“开启 / 设置后开启 / 已开启 / 关闭”完成最小操作；“高级管理”保持可见直达。浏览器自动化把文件准备、扩展连接和启用合并为能力内的一次设置，不再要求用户跳转到“组件与依赖”理解技术层级。
- **自动化中心**：管理提示词任务、计划、引用能力、Agent 绑定和运行历史。
- **设置中心**：管理模型、Agent、主题、记忆、工作区、企业消息和组件。企业消息以聊天软件列表呈现，飞书、QQ、微信使用扫码流程，钉钉和企业微信使用简化步骤；技术参数按需展开。
...
5. 在右侧抽屉预览产物，直接修订支持的文档或数据文件，再继续生成 PPTX、DOCX 或 PDF。
6. 用户明确要求后，把长期偏好写入记忆或把方法沉淀为 Skill；只有经过验证、跨任务可复用且不敏感的高价值结论才记录为经验。
7. 需要在常用聊天软件中安排任务时，扫码或按步骤连接一个企业消息渠道；切换渠道不会删除已有连接信息。
...
“文件与交付物”不是通用 Office 替代品，而是面向任务结果的安全修订面。DOCX、HTML、XLSX、CSV/TSV 和常见文本文件可以在预检通过后进入编辑；PDF、PPTX、图片、旧版 Office 以及包含高风险复杂结构的 Office 文件继续只读。

保存覆盖原文件，但始终先验证临时文件并仅保留上一版备份。外部修改通过 SHA-256 冲突检测阻止覆盖；切换上下文前统一处理未保存内容。失败必须保留用户输入、原文件和可执行的恢复路径，不能静默降级为不受保护的写入。
