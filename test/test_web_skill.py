...
class TestWebSkill(unittest.TestCase):
    ...
    def test_read_article_uses_first_successful_proxy(self, mock_request_text):
        ...
        payload = json.loads(impl.read_article("example.com/page"))
        self.assertTrue(payload["ok"])
    ...
    @patch.object(impl, "_request_text")
    def test_read_article_falls_back_to_scrapling(self, mock_request_text, mock_scrapling):
        ...
        payload = json.loads(impl.read_article("https://example.com/page"))
        self.assertTrue(payload["ok"])
        self.assertEqual(payload["extractor_used"], "scrapling")

    @patch.object(impl, "_extract_with_scrapling")
    @patch.object(impl, "_request_text")
    def test_read_article_returns_structured_error_when_all_extractors_fail(self, mock_request_text, mock_scrapling):
    ...
    @patch.object(impl, "_request_text")
    def test_read_article_respects_max_chars(self, mock_request_text):
        mock_request_text.return_value = ("# Title\n\n" + ("content " * 500), "https://markdown.new/test")
        payload = json.loads(impl.read_article("https://example.com/page", max_chars=100))
        self.assertTrue(payload["ok"])
