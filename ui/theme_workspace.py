...
class AppTitleBar(QFrame):
    """Fixed-behavior title bar whose presentation participates in theme configuration."""

    def __init__(self, window: QWidget, icon: QIcon | None = None):
        ...
        layout.setSpacing(8)
        self.logo = QLabel()
        self.logo.setObjectName("AppTitleBarLogo")
        self.logo.setFixedSize(20, 20)
        if icon and not icon.isNull():
            self.logo.setPixmap(icon.pixmap(18, 18))
        layout.addWidget(self.logo)
        brand_box = QVBoxLayout()
        brand_box.setContentsMargins(0, 0, 0, 0)
        brand_box.setSpacing(0)
        self.brand = QLabel("DeepSeek Cowork")
        self.brand.setObjectName("AppTitleBarBrand")
        self.tagline = QLabel("")
        self.tagline.setObjectName("AppTitleBarTagline")
        self.tagline.hide()
        brand_box.addWidget(self.brand)
        brand_box.addWidget(self.tagline)
        ...
        self.maximize_btn.setAccessibleName("最大化或还原窗口")
        self.close_btn.setAccessibleName("关闭窗口")
        ...
        self.system_menu_shortcut = None
        if os.name == "nt":
            self.system_menu_shortcut = QShortcut(QKeySequence("Alt+Space"), window)
            self.system_menu_shortcut.activated.connect(self.show_system_menu)
        self.refresh_theme()

    def set_brand(self, text: str):
        value = str(text or "").strip() or "DeepSeek Cowork"
        self.brand.setText(value)
        self.window.setWindowTitle(value)

    def set_tagline(self, text: str):
        self.tagline.setText(str(text or ""))
        self.tagline.setVisible(bool(str(text or "").strip()))
    ...
    def refresh_theme(self):
        self.setStyleSheet(
            f"QFrame#AppTitleBar {{ background: {DesignTokens.management_bg}; border: none; "
            f"border-bottom: 1px solid {DesignTokens.separator}; }}"
            f"QLabel#AppTitleBarBrand {{ color: {DesignTokens.text_primary}; font-size: 12px; font-weight: 600; }}"
            f"QLabel#AppTitleBarTagline {{ color: {DesignTokens.text_secondary}; font-size: 10px; }}"
            f"QToolButton {{ background: transparent; border: none; color: {DesignTokens.text_secondary}; }}"
            f"QToolButton:hover {{ background: {DesignTokens.bg_hover}; color: {DesignTokens.text_primary}; }}"
            f"QToolButton#TitleBarClose:hover {{ background: {DesignTokens.error_text}; color: {DesignTokens.text_inverse}; }}"
        )
        self.minimize_btn.setIcon(qta.icon("fa5s.minus", color=DesignTokens.icon_secondary))
        self.maximize_btn.setIcon(qta.icon("fa5s.clone" if self.window.isMaximized() else "fa5s.square", color=DesignTokens.icon_secondary))
        self.close_btn.setIcon(qta.icon("fa5s.times", color=DesignTokens.icon_secondary))

    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton and self.childAt(event.position().toPoint()) not in {
            self.minimize_btn,
            self.maximize_btn,
            self.close_btn,
        }:
    ...
    def show_system_menu(self, global_position: QPoint | None = None):
        if os.name != "nt":
            return
        import ctypes
        ...
        if not menu:
            raise RuntimeError("无法取得 Windows 系统菜单。")
        point = global_position or self.mapToGlobal(QPoint(8, self.height()))
        user32.SetForegroundWindow(hwnd)
        command = user32.TrackPopupMenu(menu, 0x0100 | 0x0002, point.x(), point.y(), 0, hwnd, None)
        if command:
            user32.PostMessageW(hwnd, 0x0112, command, 0)  # WM_SYSCOMMAND


def handle_windows_native_hit_test(window: QWidget, event_type, message):
    """Preserve native resize and Aero Snap hit testing for the frameless window."""
    if os.name != "nt" or "windows_generic_MSG" not in bytes(event_type).decode(errors="ignore"):
        return None
    import ctypes
    from ctypes import wintypes

    msg = wintypes.MSG.from_address(int(message))
    if msg.message != 0x0084 or window.isMaximized():  # WM_NCHITTEST
        return None
    x = ctypes.c_short(msg.lParam & 0xFFFF).value
    y = ctypes.c_short((msg.lParam >> 16) & 0xFFFF).value
    local = window.mapFromGlobal(QPoint(x, y))
    margin = max(6, round(8 * window.devicePixelRatioF()))
    left = local.x() < margin
    right = local.x() >= window.width() - margin
    top = local.y() < margin
    bottom = local.y() >= window.height() - margin
    hit = None
    if top and left:
        hit = 13  # HTTOPLEFT
    elif top and right:
        ...
        hit = 15
    return (True, hit) if hit is not None else None
